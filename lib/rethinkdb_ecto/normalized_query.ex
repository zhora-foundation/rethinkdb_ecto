defmodule RethinkDB.Ecto.NormalizedQuery do
  ...
  def all(query, params, _opts) do
    normalize_query(query, params)
  end

  def insert(model, fields, opts \\ []) do
    from(model)
    |> ReQL.insert(normalize_fields(fields), normalize_opts(opts))
  end

  def insert_all(model, collection, opts \\ []) do
    from(model)
    |> ReQL.insert(Enum.map(collection, &normalize_fields/1), normalize_opts(opts))
  end

  def update(model, fields, filters) do
    from(model)
    |> ReQL.get(filters[:id])
    |> ReQL.update(normalize_fields(fields))
  end
  ...
  def delete_all(query, params, opts \\ []) do
    normalize_query(query, params)
    |> ReQL.delete(normalize_opts(opts))
  end

  defp normalize_opts(opts) do
    opts
    |> List.foldl([], &normalize_opt/2)
    |> Enum.into(%{})
  end

  defp normalize_opt({returning, true}, acc), do: [{:return_changes, true}|acc]
  defp normalize_opt(kv, acc), do: acc
  ...
  defp normalize_field({k, {{y, m, d}, {hh, mm, ss, usec}}}) do
    {k, ReQL.time(y, m, d, hh, mm, ss, "Z")}
  end
  defp normalize_field({k, %Ecto.Query.Tagged{value: value} = tag}) do
    case Ecto.UUID.load(tag) do
      {:ok, binary_id} -> {k, binary_id}
      _ -> {k, value}
    end
  end
  defp normalize_field({k, %RethinkDB.Q{} = v}) do
    {k, v}
  end
  ...
  defp join(reql, %Query{joins: joins}, params) do
    Enum.reduce(joins, reql, fn %JoinExpr{on: on, source: {table, _model}}, reql ->
  ...
  defp select(reql, %Query{select: %SelectExpr{expr: {_op, _, _args} = expr}, group_bys: groups}, params) do
    ReQL.map(reql, &selectize(reql, &1, groups, evaluate_arg(expr, params), params))
  end
